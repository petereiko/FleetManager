...
class NotificationManager {
    ...
    setupDeleteModalIntegration() {
        // Listen for the delete modal form submission
        const deleteForm = document.getElementById('deleteForm');
        if (deleteForm) {
            deleteForm.addEventListener('submit', async (e) => {
                // Check if this is a notification delete (vs other entity deletes)
    ...
    async loadExistingNotifications() {
        try {
            ...
            const resp = await fetch('/api/notifications/get-recent', { credentials: 'same-origin' });
            ...
            if (list) {
                list.innerHTML = '<li class="notification-item text-center text-danger">Error loading notifications. Please refresh.</li>';
            }
        }
    }
        ...
        this.notifications.forEach(notification => {
            ...
            li.innerHTML = `
                <div class="d-flex align-items-start">
                    ...
                    <div class="flex-grow-1" onclick="window.notificationManager.markAsRead(${notification.id})" style="cursor: pointer;">
                        <div class="notification-title">${this.escapeHtml(notification.title)}</div>
                        <div class="notification-message">${this.escapeHtml(notification.message)}</div>
                        <div class="notification-time">${timeAgo}</div>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        ${!notification.isRead ? '<div><i class="fas fa-circle" style="color: #007bff; font-size: 8px;"></i></div>' : ''}
    ...
    attachDeleteListeners() {
        ...
        deleteButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const notificationTitle = btn.dataset.notificationTitle;
                this.showDeleteModal(notificationId, notificationTitle);
            });
        });
    }
    ...
    async performDelete(notificationId) {
        try {
            ...
            this.showErrorMessage('Failed to delete notification. Please try again.');
        }
    }
    ...
    showToastNotification(notification) {
        ...
        toast.innerHTML = `
            <div class="d-flex align-items-start">
                <i class="${this.getNotificationIcon(notification.type)}" style="color: ${this.getNotificationColor(notification.type)}; margin-right: 12px; margin-top: 2px; font-size: 20px;"></i>
                <div class="flex-grow-1">
                    <div style="font-weight: 600; margin-bottom: 4px; font-size: 14px;">${this.escapeHtml(notification.title)}</div>
                    <div style="font-size: 13px; color: #666;">${this.escapeHtml(notification.message)}</div>
                </div>
                <button onclick="this.closest('.toast-notification').remove()" style="border:none;background:none;color:#999;font-size:20px;line-height:1;margin-left:12px;cursor:pointer;">&times;</button>
            </div>
        `;
        document.body.appendChild(toast);
        setTimeout(() => {
    ...
    setupEventHandlers() {
        ...
        if (dropdown) {
            dropdown.addEventListener('click', () => this.updateUI());
        }
    }
}
